...
function getPathsObject(nodeSrc) {
  ...
  const outDir = path.dirname(path.resolve(options.publicDir, options.outputDir, nodeSrc))
...
function fileHasCorrectExtension(filename, extensions) {
  return extensions.length === 0
    ? true
    : extensions.map(x => x.toLowerCase()).includes(filename.split('.').pop().toLowerCase())
}
...
async function getProcessingPathsForNode(node) {
  ...
  if (node.name === 'img' && !fileHasCorrectExtension(value.data, options.imgTagExtensions)) {
...
async function createSizes(paths) {
  ...
  return (await Promise.all(sizes.map(size => resize(size, paths, meta)))).filter(Boolean)
}
...
async function replaceInComponent(edited, node) {
  ...
  replaced = insert(replaced.content, getSrcset(sizes), end + 1, end + 2, replaced.offset)
...
async function replaceImages(content, filename, preprocessors) {
  let ast
  const imageNodes = []
  ...
  try {
    content = (await svelte.preprocess(content, preprocessors, { filename })).code
...
function processFolder(folder) {
  ...
  // process
  const processingImages = images.map(filename => path.join(folder, filename)).map(processImage)
  ...
  if (options.processFoldersRecursively) {
    const folders = files.filter(fileOrFolder =>
      fs.lstatSync(path.resolve(options.publicDir, folder, fileOrFolder)).isDirectory(),
    )
    processingFolders = folders.map(nestedFolder => processFolder(path.join(folder, nestedFolder)))
  }
 ...
 */
function getPreprocessor(opts = {}, preprocessors) {
  options = {
    ...options,
    ...opts,
  }
  ...
  return {
    markup: async ({ content, filename }) => {
      ...
      return {
        code: await replaceImages(content, filename, preprocessors),
      }
    },
  }
}
